import { BaseError, FileHelper, LoggerHelper, TypeFunction, handleUpdateJoinTable } from "../../common";
...
export default class MediaRepository {
  ...
  async get(autho: AuthContentDocument, id: string): Promise<BaseMedia>
  async get(autho: AuthContentDocument, id: string, isRelation: boolean): Promise<BaseMedia>
  async get(autho: AuthContentDocument, id: string = null, isRelation: boolean = true) {
    let relations = {}
    if (isRelation) {
      relations = {
        connect: {
  ...
  async creates(autho: AuthContentDocument, inputs: BaseMedia[]): Promise<BaseMedia[]> {
    ...
    // let result = await this._repository.save(inputs);
    // return result
    return
  }
  ...
  // }
  async update(autho: AuthContentDocument, input: BaseMedia): Promise<BaseMedia> {
    let afterdata = await this._repository.save(input);
    afterdata.user = autho.user
    return afterdata
  }
  ...
  async delete(
    autho: AuthContentDocument,
    id: string, calbackCheck: (media: BaseMedia) => Promise<boolean>,
    softDelete): Promise<boolean> {
    let data = null;
    let media = await this._repository.findOne({
      where: {
        id: id,
        document: {
          id: autho.document.id,
    ...
    if (media && (!calbackCheck || (calbackCheck && await calbackCheck(media)))) {
      if (softDelete) {
  ...
  async restore(
    autho: AuthContentDocument,
    id: string,
    calback: (media: BaseMedia) => Promise<boolean>): Promise<boolean> {
    let media = await this._repository.findOne({
      where: {
        id: id,
        document: {
    ...
    if (media && (!calback || (calback && await calback(media)))) {
